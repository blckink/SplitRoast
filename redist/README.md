...
- The binaries are **not committed** to this repo. They are downloaded by
  `fetch-goldberg.ps1` (run by CI and bundled into the SplitRoast release) into
  `redist/goldberg/{x64,x86}/`.
- SplitRoast uses the emulator unmodified, as separate DLL files placed next to a
