# SplitRoast installer

This folder builds a single, shareable installer: **`output\SplitRoastSetup.exe`**.
...
3. **Publish SplitRoast self-contained** (the .NET runtime, the test window and the
   proxy are all bundled in).
4. Compile the installer to `installer\output\SplitRoastSetup.exe`.
...
`SplitRoastSetup.exe` is fully self-contained:
...
The user just runs setup, picks a folder, and launches SplitRoast. Uninstall is
available from Windows "Apps & features".

## Notes

- The installer needs administrator rights (it installs into Program Files).
- SplitRoast itself runs **without** elevation. Controller isolation writes a proxy
  ...
  library or run SplitRoast as admin if needed.
