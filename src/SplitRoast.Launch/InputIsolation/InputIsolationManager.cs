...
namespace SplitRoast.Launch.InputIsolation;
...
public sealed class InputIsolationManager
{
    ...
    public const string EnvVarName = "SPLITROAST_XINPUT_INDEX";

    /// <summary>Environment variable: path to the live "which slot to follow" file.</summary>
    public const string PadFileEnvVar = "SPLITROAST_PAD_FILE";

    /// <summary>Name of the per-instance live pad-slot file (next to the game exe).</summary>
    public const string PadFileName = "splitroast_pad.txt";

    private const string BackupSuffix = ".splitroast-bak";
    ...
    public InputIsolationManager()
    {
        ...
        string dir = Path.Combine(appData, "SplitRoast");
    ...
    {
        string path = Path.Combine(dir, "SplitRoast.XInputProxy.dll");
