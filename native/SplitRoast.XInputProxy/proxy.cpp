...
//     SPLITROAST_XINPUT_INDEX set to the physical controller index that instance
...
// Diagnostic log written next to the proxy DLL (splitroast_proxy.log). Lets us see
...
static void ProxyLog(const char* msg)
{
    ...
    if (logPath[0] == 0)
    {
        ...
        sprintf_s(logPath, MAX_PATH, "%s\\splitroast_proxy.log", modPath);
    }
...
static void EnsureInit()
{
    ...
    {
        char b[16] = { 0 };
        if (GetEnvironmentVariableA("SPLITROAST_WIN_W", b, sizeof(b)) > 0)
        {
            g_winW = atoi(b);
            GetEnvironmentVariableA("SPLITROAST_WIN_H", b, sizeof(b)); g_winH = atoi(b);
            GetEnvironmentVariableA("SPLITROAST_WIN_X", b, sizeof(b)); g_winX = atoi(b);
            GetEnvironmentVariableA("SPLITROAST_WIN_Y", b, sizeof(b)); g_winY = atoi(b);
